"use strict";
cc._RF.push(module, 'df7606xhR1GcrmjbiqMI43b', 'Grid');
...
cc.Class({
    ...
    mouseMoveHandle: function mouseMoveHandle() {
        if (this.gridHelp) {
    ...
    mouseHoverHandle: function mouseHoverHandle(mousePos) {
        ...
        if (mousePos.x < -halfNodeW || mousePos.y < -halfNodeH || mousePos.x > halfNodeW || mousePos.y > halfNodeH) {
            if (this.gridHelp) {
                this.gridHelp.active = false;
            }
            return false;
    ...
    changeGridScale: function changeGridScale(slider) {
    ...
    getGridPos: function getGridPos(x, y, isAnchorBL) {
        var halfNodeW = this.node.width * 0.5;
        var halfNodeH = this.node.height * 0.5;
        x += halfNodeW;
        y += halfNodeH;
